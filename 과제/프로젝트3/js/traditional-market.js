async function drawMap() {
  ...
  for(let i = 0; i < campingSite.length; i++) { 
    var query = encodeURI("경남" + campingSite[i].area + campingSite[i].address);
    ...
    if(location.length > 0){
      ...
      var marker = new kakao.maps.Marker({ 
        position: new kakao.maps.LatLng(location[0].address.y, location[0].address.x),
      });

      markers.push(marker);   // 마커를 배열에 추가합니다

      var infowindow = new kakao.maps.InfoWindow( {
        content: `<div style="width:150px;text-align:center;padding:6px 0;">${campingSite[i].title}(${campingSite[i].openday})</br>${campingSite[i].area} ${campingSite[i].address}</br>${location[0].x} ${location[0].y}</div>`  // 인포윈도우에 표시할 내용
      });
